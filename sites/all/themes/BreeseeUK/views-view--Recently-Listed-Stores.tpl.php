...
<div class="recent_add">
                    ...
                    <?php foreach($view->style_plugin->rendered_fields as $key => $val) { 
										...
										?>
                    <li id="<?php echo $val['name']; ?>"><a href="<?php print $base_url.'/'.drupal_get_path_alias('user/'.$val['uid']); ?>" rel="<?php print $base_url.'/store/'.$val['uid']; ?>">
                    ...
                    <div id="sss<?php echo $val['name']; ?>" class="stor_info" style="display:<?php echo $display; ?>">
                    	<h1><?php //echo $val['field_fname_value'].' '.$val['field_lname_value']; 
											echo _breesee_get_display_name($val['uid']); ?></h1>
                      <p><?php echo $val['field_about_value']; ?></p>
                    </div>  
                      <?php $display = 'none'; } ?>
                    </div>
